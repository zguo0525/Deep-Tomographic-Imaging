...
import numpy as np
from .downsampler3D import Downsampler
...
class Concat3D(nn.Module):
    ...
    def forward(self, input):
        ...
        if np.all(np.array(inputs_shapes3) == min(inputs_shapes3)) and np.all(
            ...
            for inp in inputs:
                ...
                inputs_.append(inp[:, :, :, diff3: diff3 + target_shape3, diff3:diff4 + target_shape4])
